...
// The primary, first-seen interaction on a business's public page, not a
// secondary "Not sure what to pick? Message us" link under a hero photo
// and a 3-step form. Real, live chat (same /api/web-chat endpoint, same
// session persistence, same underlying agent WebChatWidget.tsx's floating
// panel already uses elsewhere on this same page) - genuinely the same
// mechanism, just the primary entry point instead of a fallback.
// Suggested-prompt chips submit immediately on tap rather than just
// filling the input, so there's never a "now what" moment with an empty
// box staring back.
export default function ChatHero({
  ...
  /** Anchor to scroll to (the existing manual flow) when the chat backend errors - this is the primary path, so a dead end here needs a real way out, not an infinite spinner. */
  onBookingLinkFallback: string;
  /** Demo tenants only (see lib/site.ts's DEMO_SLUGS) - adds the plain-text "this is real" disclosure and the soft "start free" CTA once a visitor has actually gotten a reply. Never shown to a real business's real customers. */
  isDemo?: boolean;
}) {
  ...
  // Gates the demo-only "Start free" CTA - only after a real exchange has
  // actually happened (a booking made or a question answered), never
  // before or mid-conversation, per spec.
  ...
  useEffect(() => {
    ...
    fetch(`/api/web-chat?${new URLSearchParams({ businessId, sessionId })}`)
      ...
      .then((data) => {
        ...
        if (restored.some((m) => m.role === 'assistant')) setHasInteracted(true);
      })
      .catch(() => {})
  ...
  // this card sits in normal page flow (the site nav sits above it), tall
  // enough on a phone that the browser's own default "scroll the focused
  // input into view" can overshoot past the whole conversation. Bringing
  ...
  return (
    <div className="flex flex-col gap-2.5">
      <div
        ...
        // loads - the info panel beside it is deliberately quieter.
      ...
      >
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
          {messages.length === 0 && !thinking && (
            <div className="rounded-2xl bg-warm-surface text-ink px-3.5 py-2.5 text-[14px] leading-relaxed max-w-[85%] text-left animate-rise">
          ...
          {thinking && (
            <div className="flex justify-start animate-rise">
              <div className="text-accent-contrast rounded-2xl rounded-bl-md px-3.5 py-2 text-[14px] opacity-80 text-left" style={{ background: 'var(--accent)' }}>
          ...
          {errored && (
            ...
            // primary path, not a fallback, so a dead end here can't just
            // be an infinite typing indicator or a silent no-op.
            <div className="rounded-2xl bg-warm-surface text-ink-soft px-3.5 py-2.5 text-[13.5px] leading-relaxed max-w-[90%] text-left animate-rise">
      ...
      {isDemo && (
        <div className="px-1 flex flex-col gap-2">
          <p className="text-[12.5px] text-ink-faint">
            This is a real, working assistant for a real demo business. Ask it anything.
          </p>
          {/* Only after a real exchange has happened, never before or
              mid-conversation - a visitor evaluating the product should
              see proof first, then the offer, not the offer instead of
              proof. */}
          {hasInteracted && (
            <Link
              href="/signup"
              className="inline-flex items-center gap-1.5 self-start rounded-full px-3.5 py-2 text-[12.5px] font-semibold text-accent-contrast transition-transform hover:scale-[1.02] active:scale-95 animate-rise"
              style={{ background: 'var(--accent)' }}
            >
              Want this for your business? Start free for 14 days
