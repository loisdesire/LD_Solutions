...
// The secondary column beside ChatHero - "not deleted, not buried"
// (per the spec): today's hours/location and the top services a
// customer would otherwise have had to ask the chat for or scroll past
// the fold to find. Server-rendered, plain text - nothing here is
// interactive, so no ARIA role is needed beyond the real heading
// structure already used.
      ...
      {hoursSummary && (
        <div className="rounded-2xl bg-warm-surface border border-line px-4 py-3.5">
          ...
          <div className="flex justify-between items-baseline gap-3 py-1.5 border-b border-dashed border-line">
            ...
            {/* Wraps to two lines rather than clipping - hours are
                exactly the kind of information a customer can't afford
                to have cut off, per the spec's own edge case. */}
